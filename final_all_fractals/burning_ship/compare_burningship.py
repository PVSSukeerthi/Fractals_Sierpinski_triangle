...
# ref = np.array(Image.open("newton_float.png"),dtype=np.float64)
...
# # Compare all outputs
# for img_path in glob.glob("newton_*.png"):
...
# Load reference image (as RGB float64)
ref = np.array(Image.open("burning_ship_1000_cfloat64_11.png").convert("RGB"), dtype=np.float64)
...
for img_path in glob.glob("burning_ship_1000*.png"):
    mse, psnr, ssim = compare_images(img_path, ref)
    print(f"{img_path}: MSE={mse:.4f}, PSNR={psnr:.2f} dB, SSIM={ssim:.4f}")
